...
array = list(map(int, sys.stdin.readline().split()))
result = [i for i in array if i < x]
print(*result)
...
for _ in range(M):
    ...
    for index in range(max(0, i-1), min(N, j)):
        baskets[index] = k
...
submitters = {int(sys.stdin.readline().strip()) for _ in range(28)}
not_submitted = students - submitters
...
array = {int(sys.stdin.readline().strip()) for _ in range(10)}
result = {_ % 42 for _ in array}
...
M = max(grades)
new_grades = [(grade / M) * 100 for grade in grades]

avg = sum(new_grades) / N
